...
public class PlayerController : MonoBehaviour {
    #region 변수 목록
    public bool HasKey; //열쇠 습득하면 true로 바뀜
    public float Formal_Speed = 1f;
    ...
    //false 면 칼로 공격
    public bool CanMove;
    //false면 함수들 다 못하게, true면 움직임 함수들 다 되게
    ...
    void PlayerMove()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            {
                CanJump = false;
                GetComponent<Rigidbody2D>().velocity = new Vector2(0, Speed * 1f);
            }
            else
            {
            }
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        //키 가지고 있을 때 자물쇠 접촉하면
        if (collision.gameObject.tag == "DoorLock")
        {
            if (HasKey == true && Input.GetKeyDown(KeyCode.A))
            {
                //문 여는 모션
            }
        }
